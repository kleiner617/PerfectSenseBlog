...
import com.perfectsense.blog.repository.CommentRepository;
...
public class CommentResource {
    ...
    /**
     * GET  /comments : get all the comments by BlogID.
     *
     * @param id the pagination information
     * @return the ResponseEntity with status 200 (OK) and the list of comments in body
     */
    @GetMapping("/comments/blogID/{id}")
    @Timed
    public ResponseEntity<List<Comment>> getAllCommentsByBlogID(@PathVariable Long id) {

        log.debug("REST request to get Comments by blog ID");
        log.debug("THIS IS THE ID OF SOMETHING {}", id);
        List<Comment> result = commentRepository.findAllByBlogId(id);
        log.debug("THIS IS THE COMMENTS?? {}", result);
